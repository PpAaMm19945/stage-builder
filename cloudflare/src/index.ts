...
// ============ RHYTHM GENERATOR ROUTES (Phase 3B) ============
...
app.get('/api/rhythm/today', async (c) => {
  try {
    ...
    // Check V2 plan first
    const plan = await c.env.DB.prepare('SELECT plan_data FROM weekly_plans_v2 WHERE family_id = ? AND week_start = ?')
      .bind(user.household_id, weekStart).first<any>();

    // Get children for context in all cases
    const { results: children } = await c.env.DB.prepare(
      'SELECT * FROM students WHERE household_id = ? ORDER BY date_of_birth DESC'
    ).bind(user.household_id).all();

    if (!plan) {
      // Fallback to legacy or return empty
      return c.json({ needsPlan: true, children: children || [] });
    }
    ...
    // Find today's rhythm
    const todayRhythm = planData.days.find((d: any) => d.day === dayOfWeek);

    return c.json({
      ...(todayRhythm || { day: dayOfWeek, morning: [], evening: [], message: "Rest Day" }),
      children: children || []
    });
  } catch (e: any) {
    return c.json({ error: e.message }, 500);
...
// ============ PROGRESS TRACKING ROUTES (Phase 3B) ============
