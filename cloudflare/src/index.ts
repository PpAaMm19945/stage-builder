...
// ============================================
// PHASE 5: AI SCHEDULE ADJUSTMENT
// ============================================

app.post('/api/rhythm/readjust', async (c) => {
  try {
    const user = requireAuth(c);
    const { prompt, weekStart, currentSchedule } = await c.req.json();

    if (!prompt || !weekStart) {
      return c.json({ error: 'Prompt and weekStart are required' }, 400);
    }

    // 1. Fetch current plan if not provided or just to verify
    const { results } = await c.env.DB.prepare(
      'SELECT * FROM weekly_plans WHERE parent_id = ? AND week_start = ?'
    ).bind(user.id, weekStart).all();

    const planRow: any = results[0];
    if (!planRow) {
      return c.json({ error: 'Plan not found for this week' }, 404);
    }

    const currentPlan = JSON.parse(planRow.plan_json);

    // 2. Prepare AI Context
    const systemPrompt = `
      You are an expert family scheduler. 
      The user wants to adjust their daily rhythm for today or this week.
      
      Current Schedule Slots (JSON):
      ${JSON.stringify(currentPlan.slots.slice(0, 20))} ... (truncated for context if needed)

      User Request: "${prompt}"

      Instructions:
      1. Interpret the user's intent (e.g., "Shift 1 hour later", "Remove outdoor time today", "Swap morning and afternoon").
      2. Return ONLY the modified JSON array of slots.
      3. Maintain the same JSON structure for each slot.
      4. Ensure times are formatted as HH:00 or HH:30.
      5. Do not explain. Return JSON only.
    `;

    // 3. Call AI
    // Using Llama-3-8b-instruct or similar
    const messages = [
      { role: "system", content: "You are a JSON-only API. You output raw JSON array of schedule slots." },
    ...
    // 4. Parse AI Output
    // This is risky if AI returns markdown or text. We need robust parsing.
    let newSlots = [];
    try {
      // Attempt to extract JSON array
      const text = aiResponse.response;
      const jsonMatch = text.match(/\[.*\]/s); // Naive regex
      if (jsonMatch) {
        newSlots = JSON.parse(jsonMatch[0]);
      } else {
        // Fallback: maybe it returned just the array
        newSlots = JSON.parse(text);
      }
    } catch (e) {
      console.error("Failed to parse AI response", e);
      return c.json({ error: 'AI failed to generate valid JSON' }, 500);
    }

    if (!Array.isArray(newSlots) || newSlots.length === 0) {
      return c.json({ error: 'Invalid AI output' }, 500);
    }

    // 5. Update Plan Obect
    // We only update the slots in the plan
    const updatedPlan = { ...currentPlan, slots: newSlots };

    // 6. Save to DB
    await c.env.DB.prepare(`
      UPDATE weekly_plans 
      SET plan_json = ?, generated_at = datetime('now')
      WHERE id = ?
    `).bind(JSON.stringify(updatedPlan), planRow.id).run();
