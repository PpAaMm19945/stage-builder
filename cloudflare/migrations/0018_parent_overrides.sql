-- Phase 2: Parent Override Schema
-- Stores structured constraints derived from parent input
-- AI parses free-text → structured JSON, but never alters curriculum logic
...
-- Weekly Time Model
-- ============================================
-- Captures parent's available time for weekly planning
CREATE TABLE IF NOT EXISTS weekly_time_model (
  ...
  preferred_times TEXT NOT NULL DEFAULT '["morning"]',  -- JSON array: morning, afternoon, evening
  max_sessions_per_day INTEGER NOT NULL DEFAULT 2,
  field_trip_days TEXT DEFAULT '[]',  -- JSON array of days reserved for outings
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  FOREIGN KEY (parent_id) REFERENCES users(id) ON DELETE CASCADE
);
...
-- ============================================
-- Stores deterministically-generated weekly plans
...
-- ============================================
-- Curriculum Explanations Log (for transparency)
-- ============================================
-- Logs AI-generated explanations for audit/debugging
