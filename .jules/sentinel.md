...
## 2026-10-24 - Missing Rate Limiting on Library Endpoints
**Vulnerability:** The public library endpoints (`/api/books`, `/api/series`, `/api/series/:id`) perform expensive R2 list operations but lacked rate limiting. This allowed anonymous users to trigger resource exhaustion (DoS) and increase Class A/B operation costs by flooding these endpoints.
**Learning:** Public "read-only" endpoints that trigger heavy backend operations (like listing thousands of files) are valid DoS targets even if they don't modify data. Rate limiting should be applied to all public APIs, not just authentication or write endpoints.
**Prevention:** Applied IP-based rate limiting (60 req/min) to all public library discovery endpoints.
