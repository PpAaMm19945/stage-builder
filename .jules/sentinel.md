...
## 2026-08-30 - Missing Rate Limiting on Public Endpoints
**Vulnerability:** Publicly accessible endpoints (e.g., `/api/books/*`) lacked rate limiting. This allowed unauthenticated attackers to perform resource exhaustion attacks (DoS) or drive up costs by spamming requests to R2-backed endpoints.
**Learning:** Authenticated routes often get rate-limited by user ID, but public routes are frequently overlooked. IP-based rate limiting is essential for any public-facing API to prevent abuse.
**Prevention:** Implement an IP-based rate limiting middleware (`createRateLimiter`) and apply it to all public API routes. Use namespaced keys (e.g., `library:${ip}`) to avoid collisions with other rate limits (like login).
