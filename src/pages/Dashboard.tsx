...
export default function Dashboard() {
  ...
  const handleUpNextAction = useCallback((item: RhythmItem) => {
    if (item.type === 'book' && todaysBook) setSelectedBook(todaysBook);
    setShowFullDay(true);
  }, [todaysBook]);

  const handleUpNextExpand = useCallback(() => setShowFullDay(prev => !prev), []);
  ...
  const handleBookClick = useCallback(() => {
    if (todaysBook) setSelectedBook(todaysBook);
  }, [todaysBook]);
  ...
  const timelineItems = useMemo(() => {
    ...
    orderedContexts.forEach(context => {
      const groupItems = groups[context];
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-12 px-4 sm:px-0">
      ...
      {isToday && (
        <UpNextCard
          item={nextItem}
          onAction={handleUpNextAction}
          onExpand={handleUpNextExpand}
